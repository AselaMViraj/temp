...
hardware_interface::CallbackReturn DiffDriveArduino::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // Discard any stale data in the serial buffer
  arduino_.sendEmptyMsg();

  // Reset timing so the first deltaSeconds in read() is accurate
  time_ = std::chrono::system_clock::now();

  // Pre-initialize wheel positions from the actual encoder values.
  // The ZLAC8015D encoder registers hold an absolute position (e.g. ~37000 counts).
  // Without this, the first read() sees a jump from pos=0 to pos=67+ radians,
  // causing diff_drive_controller to compute a huge velocity spike and corrupting SLAM.
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  double left_rpm = l_wheel_.cmd * 60.0 / (2.0 * M_PI);
  double right_rpm = r_wheel_.cmd * 60.0 / (2.0 * M_PI);
  arduino_.setMotorValues(static_cast<int>(left_rpm), static_cast<int>(right_rpm));
